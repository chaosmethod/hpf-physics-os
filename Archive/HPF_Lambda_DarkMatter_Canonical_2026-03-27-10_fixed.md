...
Both results emerged from the same derivation chain without being separately targeted. No parameters were imported from observation. The residuals are consistent with \(L_{\rm vac} = 10^{-19}\ {\rm m}\) being the current instrumental limit rather than the exact substrate value.
...
**Consequence:** \(L_{\rm vac}\) ceases to be a free parameter. The bandwidth cutoff \(\Lambda_{\rm HPF} = 1/L_{\rm vac}\) derives from \(c\) alone. The current value \(L_{\rm vac} \sim 10^{-19}\ {\rm m}\) is the instrumental resolution limit, not a substrate fundamental.
...
Residual is consistent with \(L_{\rm vac}=10^{-19}\ {\rm m}\) being the current instrumental limit. Exact closure is expected when \(L_{\rm vac}\) is derived from substrate fundamentals (Section 10 item 2).
...
Derived: \(26.3\%\)  
Residual: \(0.5\%\) — consistent with instrumental \(L_{\rm vac}\) placeholder.
...
|---|---|---|
| LIV shield via coherence blur | Lattice anisotropy corrections of order \(\epsilon\), suppressed for \(l_O \gg L_{\rm vac}\) by the same kernel | Follows from continuum emergence theorem — closed |
| Continuum emergence | Observable bound derived; \(d_{\rm eff}=1.4\) derived from BCC diagonal structure \(\times \lambda_{\rm blur}\) | All four conditions met — candidate-locked |
...
This fixes the recursion depth as a pure radial growth count. The earlier insertion of a bipartite \(1/2\) factor into the radial identity was incorrect and has been removed. Bipartite structure remains in the occupancy/update logic, not in the radial scale ratio. The \(e^{3\pi/2}\) streaming factor remains a separate substrate-native suppression in the \(\Lambda\) chain.
...
This derivation package should be cross-referenced in Volume II theorem track and Volume III provenance map.
