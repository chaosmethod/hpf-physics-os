...
# 8. Post-March-20 additions — 2026-03-27

## 8.1 Symbol index

`Symbol_index.md` has been updated and formally added to the canonical package. It was previously a standalone document in the older repo structure, not referenced by Volume III. It now travels with the Volume I–III package.

Key additions to the symbol index (2026-03-27 update):
- $c$ (substrate propagation limit) — the lattice identity $L_{vac} \equiv c \cdot t_{sched}$
- Section 7 (Vacuum Sector Symbols) — $L_{vac}$, $\varphi$, $f_{coh}$, $\alpha_{vac}$, $n_{spiral}$, $\kappa_{BCC}$, $\varepsilon$, $d_{eff}$
- Section 8 updated — blur threshold $S_f = 1.05$ added; phase boundary relationship documented

## 8.2 Lambda and dark matter derivation package

`HPF_Lambda_DarkMatter_Canonical_2026-03-27.md` is a new canonical derivation document added to the package on 2026-03-27.

It derives the cosmological constant $\Lambda$ and the dark matter fraction from the BCC Fibonacci substrate geometry using no phenomenological fit parameters in the substrate derivation chain.
...
- $L_{vac}$ derived: $R_H / (2\varphi^{n_{spiral}}) = 1.465 \times 10^{-20}$ m (bipartite 1/2 factor)
- $\alpha_{vac} = \sqrt{f_{coh}}$ (governor transfer theorem)
- $1 - \alpha_{vac} = 26.3\%$ = dark matter fraction (governor-converted deferred substrate load)
- $e^{3\pi/2}$ suppression from 3D BCC streaming dimensionality
- $d_{eff} = \lambda_{blur} \times (4/3) = 1.05 \times (4/3) = 1.4$ — phase boundary relationship derived from BCC 4-diagonal structure
- Continuum emergence theorem: $\varepsilon = (1-\zeta(S_f)) \cdot (L_{vac}/\ell_O)^{1.4}$

**Truth-status:** Candidate-locked. All three formal open items are closed. Awaiting independent verification before upgrading to theorem-complete.

**Relationship to existing canon:** This package extends the HPF regulatory architecture into the vacuum sector. It does not modify or supersede Volume I–III content. Volume I–III remain the primary canon for the regulatory framework. The Lambda/dark matter package is a downstream derivation using the substrate architecture established in Volume II.
...
2. `volume-2-microscopic-derivation.md` — microscopic and executable line
3. `volume-3-provenance-map.md` — provenance and live-reference map
4. `Symbol_index.md` — canonical symbol reference (now travels with volumes)
5. `qprca-v0.2.8.py` — front active executable
6. existence-sensor protocol note and test artifacts
7. `HPF_Lambda_DarkMatter_Canonical_2026-03-27.md` — vacuum sector derivation package
