...
| $d_{eff}$ | Effective redistribution dimension | $d_{eff} = \lambda_{blur} \times (4/3) = 1.4$ | Rounded structural marker from BCC diagonal structure $\times$ blur threshold |
...
| \(d_{eff}=1.4\) | Rounded structural instability marker | \(1.05\times 4/3\), blur threshold times BCC diagonal/branch structure | Derived / candidate-locked structural marker | Rounded transition marker |
